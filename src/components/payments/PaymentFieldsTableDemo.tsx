...
const WEBHOOK_FIELD_DESC: Record<string, string> = {
...
type WebhookSnapshot = { data?: Record<string, unknown> } | null | undefined;
interface PaymentFieldsTableDemoProps {
  data: WebhookSnapshot;
}

export default function PaymentFieldsTableDemo({
  data,
}: PaymentFieldsTableDemoProps) {
  let webhookData: Record<string, unknown> | null = null;
  if (data && data.data) {
    const { orderCode, description, ...restData } = data.data as Record<
      string,
      unknown
    >;
    const merged = { orderCode, description, ...restData } as Record<
      string,
      unknown
    >;
        ...
        <Paper sx={{ overflow: "hidden" }}>
          ...
          <TableContainer>
            ...
            >
              ...
              <TableBody>
                {webhookData ? (
                  Object.keys(webhookData as Record<string, unknown>).map(
                    (key) => (
                      <TableRow key={key}>
