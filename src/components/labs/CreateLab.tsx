...
export default function CreateLab() {
  ...
  const validateFolder = (paths: string[]) => {
    ...
    const allImagesUnderImages = imagePaths.every((p) => p.toLowerCase().startsWith("images/"));
  ...
  const collectDropped = async (dt: DataTransfer): Promise<FolderItem[]> => {
    ...
    const canUseEntries = !!first && typeof first.webkitGetAsEntry === "function";
    if (canUseEntries) {
        ...
        if (entry.isFile) {
          ...
          const readAll = async () => {
            ...
            for (const ent of entries) {
              await walk(ent, prefix ? `${prefix}/${dirEntry.name}` : dirEntry.name);
            }
            await readAll();
          };
          await readAll();
      ...
      for (const it of items) {
        if (it.kind === "file") {
          const dtItem = it as DataTransferItem & { webkitGetAsEntry?: () => FileSystemEntry | null };
  ...
  function stripCommonRoot(items: FolderItem[]): FolderItem[] {
          ...
          const allPrefixed = parts.every((arr) => arr.length > 1 && arr[0] === firstSeg);
  ...
  const onSelectFolder = (list: FileList | null) => {
          ...
          const norm = items.map((it) => ({ ...it, path: it.path.replace(/\\/g, "/") }));
          const stripped = stripCommonRoot(norm);
          setFolderPayload(stripped);
          validateFolder(stripped.map((i) => i.path));
        };
  ...
  const onAddImages = (list: FileList | null) => {
          ...
          setImages((prev) => {
            ...
            if (deduped.length < incoming.length) {
              toast.warn("Some images were skipped because they already exist by name.");
            }
            return [...prev, ...deduped];
          });
        };
  ...
  const insertImagesAtCaret = (files: File[]) => {
          ...
          const snippet =
            "\n" + imageFiles.map((f) => `![${toAlt(f.name)}](images/${f.name})`).join("\n") + "\n";
  ...
  const onSubmit = async () => {
          ...
          try {
            ...
            if (folderPayload.length) {
              ...
              filesToUpload = folderPayload.map((f) => ({ file: f.file, path: f.path }));
            } else {
              const contentBlob = new Blob([markdown], { type: "text/markdown" });
              filesToUpload.push({ file: new File([contentBlob], "index.md", { type: "text/markdown" }), path: "index.md" });
              for (const img of images) filesToUpload.push({ file: img, path: `images/${img.name}` });
            }
            ...
            await supabase.storage.from(BUCKET).upload(`${slug}/meta.json`, metaBlob, { upsert: true, cacheControl: "60" });

            const mdPath = `${slug}/index.md`;
            const { data: pubUrlData } = supabase.storage.from(BUCKET).getPublicUrl(mdPath);
            ...
            const dto = {
              ...
              categoryId: CATEGORY_ID_MAP[category as LabType],
            };
  ...
  useEffect(() => {
          ...
          const run = async () => {
            ...
            try {
              const idx = folderPayload.find((it) => it.path.toLowerCase() === "index.md");
              if (idx) {
          ...
          >
            <textarea
              ...
              className={`w-full rounded-xl border px-4 py-3 focus:outline-none focus:ring-2 min-h-[60vh] ${
                isEditorDragging ? "border-blue-400 bg-blue-50 focus:ring-blue-400" : "border-gray-300 focus:ring-blue-400"
              }`}
              value={markdown}
                ...
                <ReactMarkdown
                  ...
                  components={{
                    ...
                    img({ ...props }) {
                      ...
                      const normalized = srcRaw.replace(/^\.\{1,2\}\//, "").replace(/^\//, "");
                      const nameOnly = normalized.startsWith("images/") ? normalized.slice(7) : normalized;
                      const local = images.find((f) => `images/${f.name}` === normalized || f.name === nameOnly);
                      if (local) {
                        const url = URL.createObjectURL(local);
                        return <img src={url} alt={props.alt as string} onLoad={() => URL.revokeObjectURL(url)} className="max-w-full rounded" />;
                      }
  ...
  return (
          <div className="max-w-6xl mx-auto">
              ...
              <div className="inline-flex rounded-xl bg-gray-100 p-1">
                <button
                  className={`cursor-pointer px-4 py-2 rounded-lg text-sm font-semibold ${
                    activeTab === "upload" ? "bg-white shadow text-gray-900" : "text-gray-600 hover:text-gray-800"
                  }`}
                ...
                <button
                  className={`cursor-pointer px-4 py-2 rounded-lg text-sm font-semibold ${
                    activeTab === "editor" ? "bg-white shadow text-gray-900" : "text-gray-600 hover:text-gray-800"
                  }`}
            ...
            {activeTab === "upload" && (
              <div className="mb-6">
                <h3 className="text-xl font-semibold text-gray-900 mb-2">Upload Folder</h3>
                <p className="text-gray-600 mb-4">
                  Import an existing lab by selecting a folder that contains <code>index.md</code> and optionally an <code>images/</code> directory for assets.
                </p>
                <div className="p-6 rounded-2xl bg-white border border-gray-200">
                  <div className="mb-4 rounded-xl border bg-gradient-to-br from-gray-50 to-white p-4">
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <h4 className="text-base font-semibold text-gray-800">Folder structure</h4>
                    </div>
                    <ul className="text-sm text-gray-700 space-y-1">
                      ...
                      <li className="ml-6 flex items-center gap-2">
                        <span className="text-gray-500">📝</span>
                        <code>index.md</code> <span className="text-gray-500">— Markdown content</span>
                      </li>
                      <li className="ml-6 flex items-center gap-2">
                        <span className="text-gray-500">📂</span>
                        <span className="font-medium">images/</span> <span className="text-gray-500">— All images (optional, if present all images must be inside)</span>
                      </li>
                    ...
                    <p className="text-gray-600 mt-3 text-sm">
                      Use relative paths inside Markdown, e.g. <code>![alt](images/image-1.png)</code>.
                    </p>
                  </div>

                  <div
                    className={`rounded-2xl p-6 transition-colors duration-150 ${
                      isDragging ? "bg-blue-50 border-2 border-blue-400" : "bg-gray-50 border-2 border-dashed border-gray-300"
                    }`}
                  ...
                  >
                    ...
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                      <div className="flex items-center gap-3">
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center text-xl ${isDragging ? "bg-blue-100 text-blue-700" : "bg-white text-gray-700 border"}`}>
                          📁
                        </div>
                        <div>
                          <div className="font-semibold text-gray-800">Drop your lab folder</div>
                          <div className="text-sm text-gray-600">Or click to pick a folder. We’ll import index.md and images/</div>
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        <button className="cursor-pointer px-4 py-2 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-semibold" onClick={() => uploadInputRef.current?.click()}>
                          Choose Folder
                        </button>
                      </div>
                    </div>
                    ...
                    {folderPayload.length > 0 && (
                      <div className="mt-4 text-sm text-gray-700">
                        <div className="flex items-center justify-between">
                          <div className="font-semibold">Selected files ({folderPayload.length})</div>
                          <button
                          ...
                          <table className="min-w-full text-sm">
                            ...
                            <tbody>
                              {folderPayload.slice(0, 150).map((it, i) => {
                                const isImg = /\.(png|jpe?g|gif|webp|svg)$/i.test(it.path);
                                const url = isImg ? urlsRef.current.get(it.path) : undefined;
                                return (
                                  <tr key={i} className="border-t align-middle">
                                    <td className="py-1 pr-3">
                                      <div className="flex items-center gap-2">
                                        {isImg && url ? (
                                          <img src={url} alt="img" className="w-8 h-8 object-cover rounded" />
                                        ) : (
                                          <span className="text-gray-500">{it.path.endsWith(".md") ? "📝" : "📄"}</span>
                                        )}
                                        <span className="truncate max-w-[380px]" title={it.path}>
                                          {it.path}
                                        </span>
                                      </div>
                                    </td>
                                    <td className="py-1 pr-3 text-gray-600">{isImg ? "Image" : it.path.endsWith(".md") ? "Markdown" : "File"}</td>
                                    <td className="py-1 pr-3 text-gray-600">{(it.file.size / 1024).toFixed(1)} KB</td>
                                  </tr>
                                );
                              })}
                    ...
                    <div className="mt-5">
                      <div className="flex items-center justify-between mb-2">
                        <div className="font-semibold text-gray-800">Markdown Preview (index.md)</div>
                        <button
                      ...
                      <div className="rounded-xl border bg-white p-3 min-h-[160px]">
                        {isReadingIndex ? (
                          <div className="text-sm text-gray-500">Reading index.md…</div>
                        ) : uploadMarkdown ? (
                          <div className="markdown-body max-w-none">
                            <ReactMarkdown
                              remarkPlugins={[remarkGfm, remarkBreaks]}
                              ...
                              components={{
                                ...
                                img({ ...props }) {
                                  ...
                                  const normalized = raw.replace(/^\.\{1,2\}\//, "").replace(/^\//, "");
                                  let url = urlsRef.current.get(normalized);
                                  if (!url && normalized.includes("/")) {
                                    const withoutFirst = normalized.substring(normalized.indexOf("/") + 1);
                                    url = urlsRef.current.get(withoutFirst);
                                  }
                                  if (!url) url = urlsRef.current.get(raw);
                                  if (url) {
                                    return <img src={url} alt={(props.alt as string) || ""} className="max-w-full rounded" />;
                                  }
                                  return <img {...props} className="max-w-full rounded" />;
                                },
                              }}
                          ...
                          <div className="text-sm text-gray-500">No index.md found at folder root.</div>
                        )}
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="absolute inset-0 p-4 md:p-6 lg:p-8 overflow-auto">
                      <div className="mx-auto max-w-[1400px] bg-white rounded-xl shadow-2xl p-4 md:p-5 lg:p-6">
                        <div className="flex items-center justify-between mb-3">
                          <div className="text-lg font-semibold text-gray-900">Editor — Full Screen</div>
                          <button
                            className="cursor-pointer inline-flex items-center gap-2 px-3 py-1.5 rounded-lg border bg-white hover:bg-gray-50 text-gray-700"
                ...
                <div className="mt-4">
                  ...
                  {images.length > 0 ? (
                    <ul className="mt-2 grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm text-gray-700">
                      {images.map((img, i) => (
                        <li key={i} className="flex items-center justify-between rounded-lg border px-3 py-2">
                          ...
                          <button className="cursor-pointer text-red-600 text-xs" onClick={() => onRemoveImage(i)}>
                            Remove
                          </button>
                        </li>
                      ))}
                    ...
                    <p className="text-sm text-gray-600 mt-2">
                      No images added yet. You can reference them as <code className=" text-red-500 font-semibold">images/your-file.png</code> in Markdown.
                    </p>
                  )}
                </div>
              </div>
            ...
            <div className="flex items-center justify-end gap-3 mt-6">
              <button className="cursor-pointer px-5 py-2 rounded-xl bg-gray-100 text-gray-800 hover:bg-gray-200" onClick={resetForm}>
                Reset
              </button>
              <button
                disabled={isSubmitting || !hasRequiredMeta || (folderPayload.length > 0 && uploadErrors.length > 0)}
                className={`cursor-pointer px-5 py-2 rounded-xl text-white font-semibold ${
                  isSubmitting || !hasRequiredMeta || (folderPayload.length > 0 && uploadErrors.length > 0) ? "bg-blue-300" : "bg-blue-600 hover:bg-blue-500"
                }`}
                onClick={onSubmit}
              >
